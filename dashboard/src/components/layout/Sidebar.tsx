...
export default function Sidebar() {
  ...
  // Check if a specific child route is active (exact match)
  function isChildActive(href: string): boolean {
    return pathname === href;
  }

  // Check if a section (parent) is active — any child or the parent path itself
  function isSectionActive(item: NavItem): boolean {
    // For items with children, check if any child href matches
    if (item.children) {
      if (item.children.some(child => pathname === child.href || pathname.startsWith(child.href + '/'))) {
        return true;
      }
      // Also check the base path for the section
      const basePath = item.href.split('/').slice(0, 2).join('/');
      return pathname.startsWith(basePath);
    }
    // For leaf items, exact match
    return pathname === item.href;
  }

  return (
    <aside className="w-60 min-h-screen bg-surface-low flex-shrink-0 flex flex-col">
      {/* Logo header — gradient from primary */}
        ...
        {navItems.map((item) => {
          ...
          return (
            <div key={item.label} className="mb-1">
              ...
              {/* Children — ALWAYS visible when section active */}
